...
export default function LearningSupportPage(){
  ...
  return <div className="fp-page">
    ...
    <section><h3>طلبات الدعم</h3><div className="fp-list">{requests.length===0?<div className="fp-empty">لا توجد طلبات</div>:requests.map(r=><article className="fp-card" key={r.id}>
      ...
      {r.status==='pending'&&<button className="btn outline small" onClick={()=>cancelLearningSupportRequestWeb(r.id).then(load).catch(e=>setError(e.message))}>إلغاء الطلب</button>}
    </article>)}</div></section>
