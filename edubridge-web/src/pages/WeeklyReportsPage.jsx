...
export default function WeeklyReportsPage(){
  ...
  return <div className="fp-page">
    ...
    <section className="fp-grid">{reports.length===0?<div className="fp-empty">لا توجد تقارير لهذا الطفل</div>:reports.map(r=><article className="fp-card" key={r.id}>
      ...
      <div className="fp-grid"><div><div className="fp-stat">{r.lessons_completed}/{r.lessons_total}</div><small>الدروس</small></div><div><div className="fp-stat">{r.homework_submitted}/{r.homework_assigned}</div><small>الواجبات</small></div><div><div className="fp-stat">{r.therapy_sessions_attended}/{r.therapy_sessions_scheduled}</div><small>اجتماعات الدعم</small></div></div>
